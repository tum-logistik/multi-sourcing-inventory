...
def add_in_additional_var(state_obj, action):
    ...
    if s_a_tuple not in u:
        tau_s_a = sourcingEnv.compute_event_arrival_time(a, state_obj = state_obj)
        state_cost = cost_calc_unit(state_obj)*tau_s_a
        action_cost = np.sum(np.multiply(sourcingEnv.procurement_cost_vec, action))*tau_s_a

        cost = (state_cost + action_cost)
        # cost = state_cost/tau_s_a
...
for s in poss_states_objs:
    for a in action_space:
        s_a_tuple = (s.get_nested_list_repr(), repr(list(a)))
        if s_a_tuple not in u:
