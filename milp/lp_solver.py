...
possible_state_tuples = list(itertools.product(state_s, state_backorders, state_onoff, state_onoff))
poss_states = [[x[0], np.array(list(x[1])), np.array(list(x[2]))] for x in possible_state_tuples]

action_space = list(itertools.product(range(sourcingEnv.action_size), range(sourcingEnv.n_suppliers))) 
...
for state in poss_states:
    for a in action_space:
        ...
        a_rep = repr(a)
        x[state_rep, a_rep] = m.addVar(obj= cost_calc(state_rep, a), name='x-'+str(state)+"-"+str(a))
